...
@testset "Heading - Circular Path" begin
    # Kite flying in a circle in a vertical plane
    elevation = deg2rad(45)
    radius = 100.0
    circle_radius = 20.0
    center = [radius * cos(elevation), 0.0,
    ...
    for angle in angles
        ...
        # Expected: manual projection onto tangent plane
        e_x_t = R_t_to_w' * e_x
        expected = atan(e_x_t[2], e_x_t[1])

        R_b_to_w = hcat(e_x, zeros(3), zeros(3))
        @test calc_heading(R_b_to_w, wing_pos) ≈
            expected atol = 1e-10
    end
end
end
...
@testset "Heading - Horizontal Circle" begin
    # Kite flying a horizontal circle at constant elevation
    elevation = deg2rad(45)
    radius = 100.0
    n_points = 24
    ...
    for azimuth in azimuths
        ...
        R_b_to_w = hcat(e_x, zeros(3), zeros(3))
        heading_val = calc_heading(R_b_to_w, wing_pos)

        # Azimuthal tangent → heading ≈ ±π/2
...
@testset "Heading - Special Cases" begin
    ...
    e_x = R_t_to_w[:, 1]
    R_b_to_w = hcat(e_x, zeros(3), zeros(3))
    @test calc_heading(R_b_to_w, wing_pos) ≈ 0.0 atol=1e-10

    # e_x along y_t (azimuthal) → heading = π/2
    e_x = R_t_to_w[:, 2]
    R_b_to_w = hcat(e_x, zeros(3), zeros(3))
    @test calc_heading(R_b_to_w, wing_pos) ≈ π/2 atol=1e-10

    # Roundtrip: construct e_x at known heading angles
    for h in [0, π/6, π/4, π/3, π/2, 2π/3, π, -π/6, -π/4]
        e_x_w = R_t_to_w * [cos(h), sin(h), 0.0]
        R_b_to_w = hcat(e_x_w, zeros(3), zeros(3))
        @test calc_heading(R_b_to_w, wing_pos) ≈ h atol=1e-10
    end
end
end
