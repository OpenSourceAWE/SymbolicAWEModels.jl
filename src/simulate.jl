...
function sim_oscillate!(
    ...
    if prn
        @info "Simulating using oscillating steering " *
              "inputs\n" *
    ...
    for step in 1:steps
        ...
        steering = steering_magnitude *
            cos(2π * steering_freq * t + bias)
        set_values[step, :] = [0.0, steering, -steering]
    end

    return sim!(sam, set_values; dt, total_time,
                vsm_interval, prn, lin_model, y_op,
                torque_damp)
end
