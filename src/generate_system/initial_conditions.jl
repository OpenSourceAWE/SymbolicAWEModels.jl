...
bound variable is routed by how it survived `mtkcompile`: a surviving unknown's
`u0` is written directly (`setu`) — under `build_initializeprob=false` there is
no init solve to apply its `Initial(x)` parameter, so setting that parameter
alone would leave `u0` stale — while an observed variable that `mtkcompile`
solves as an init constraint is set through its `Initial` parameter. Variables
