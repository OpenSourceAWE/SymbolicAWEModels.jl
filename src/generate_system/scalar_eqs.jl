...
function scalar_eqs!(
    ...
    for wing in wings
        ...
        # x_t = elevation dir (away from zenith), y_t = azimuthal.
        # heading = 0 when e_x aligns with x_t (nose toward GS).
        ...
        eqs = [
            ...
            heading[wing.idx] ~ atan(heading_t_2, heading_t_1)
            turn_rate[:, wing.idx] ~
                R_v_to_w[:, :, wing.idx]' *
            ...
            azimuth_acc[wing.idx] ~
                ...
                norm([x, y])
            course[wing.idx] ~ atan(course_t_2, course_t_1)
